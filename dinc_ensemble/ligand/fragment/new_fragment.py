...
class DINCFragment:
    ...
    def to_df_info(self, 
        ...
        frag_dofs = []
        frag_pdbqts = []
        for i, elem in enumerate(self.fragments[start:end]):
            
            suffix = i
            if i == len(self.split_frags)-1:
                suffix = "full"
            #elem_path_svg = p / (elem_molkit_mol.name+"_frag_{}.svg".format(suffix))
            elem_path_pdbqt = p / (self._molecule.name+"_frag_{}.pdbqt".format(suffix))
